...
Generally, discordance can be described using two sets $A$ and $B$ by comparing their elements
...
The `hpe()` function returns a numeric value for $H_{+}$ given $A,B$ or $D,L$, or it returns a list with the additional parameters (alpha and gamma) specified.
...
A logical which indicates whether `hpe()`  should include the values for $\alpha_{W}$ and $\alpha_{B}$, the portion of dissimilarities that are either between within and between-cluster.
...
1. A data matrix $D$ with n rows (observations) corresponding to a vector L of length $n$.

The `hpe()` function returns a numeric value for $H_{+}$ given $D,L$.

## Additional arguments

In addition $D,L$, the user provides `hpe()` with two additional arguments: `p` and `alg`
...
Should be around 1-5% of the number of observations in the dataset (does this need to be verified????)
