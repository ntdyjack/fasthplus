...
(maybe put the computational figure here?)
...
The primary difference is that `hpe()` requires calculation of the full $D$ matrix as either an argument or an internal routine, while `hpb()` calculates sub-matrices of $D$ per iteration
...
If calculation of $D$ is practical or fine control of numerical accuracy of the estimate is requir we recommend the utilization of the `hpe()` (H Plus Estimator) function.
...
```{r}
hpe(D=d,L=l) # D,L formulation
intCriteria(traj=t(dat),part=as.integer(l),crit='G_plus') #G+ value
hpb(d=dat,
```
