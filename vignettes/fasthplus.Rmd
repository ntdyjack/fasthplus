...
For further exploration of discordance, these estimators ($G_{+}$ and $H_{+}$), as well as their theoretical properties, please see Dyjack et al. (2022).

# $H_{+}$ estimation in `fasthplus` with `hpe()` and `hpb()`

The primary workhorse functions in `fasthplus` are the `hpe()` (H Plus Estimator) and `hpb()` (H Plus Bootstrap) functions.

In general, the computationally expensive aspects of explicit $H_{+}$ calculation are calculation of the dissimilarity matrix $D$ and the number of discordant pairs $s$ (Dyjack et al. 2022).
...
Both `hpe()` and `hpb()` are estimation procedures for $H_{+}$ which circumnavigate the most costly step: explicit calculation of $s$ by comparing each within-cluster distance to every between-cluster distance.

The two functions differ in their treatment of the dissimilarity matrix $D$, yielding distinct computational and theoretical advantages for each.

On a modest machine, calculating and storing $D$ becomes cumbersome at around 10000 observations with 1000 features.

The primary difference is that `hpe()` requires calculation of the full $D$ matrix as either an argument or an internal routine, while `hpb()` calculates sub-matrices of $D$ per iterate.

The more explicit `hpe()` gives a numerical bound for the accuracy as a function of $p$, while the convegence of `hpb()` at this point is largely heuristic.

For smaller datasets, the `fasthplus` package includes two fast algorithms to estimate $H_{+}$.

If calculation of $D$ is practical or fine control of numerical accuracy of the estimate is required we recommend the utilization of the `hpe()` (H Plus Estimator) function.
...
The primary workhorse function in `fasthplus` is the `hpe()` (H Plus Estimator) function.
...
The `hpe()` function returns a numeric value for $H_{+}$ given $A,B$ or $D,L$.
...
# Using `hpb()` in `fasthplus` for larger datasets or less stringent accuracy requirements

If the calculation of $D$ 
